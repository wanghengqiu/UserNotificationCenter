...
-(void)createNotification:(UIButton *)sender
{
    ...
    if (imageFilePath) {
        UNNotificationAttachment *imageAttatch = [UNNotificationAttachment attachmentWithIdentifier:@"imageAttach" URL:[NSURL fileURLWithPath:imageFilePath] options:nil error:nil];
        
        if (imageFilePath) {
            content.attachments = @[imageAttatch];
        }
    }
    
    NSMutableArray *array = [[NSMutableArray alloc]initWithCapacity:5];
    UNNotificationAction *actionA = [UNNotificationAction actionWithIdentifier:@"IdentifierNeedUnUnlock" title:@"需要解锁" options:UNNotificationActionOptionAuthenticationRequired];
    [array addObject:actionA];
    ...
    [array addObject:actionB];
    
    UNNotificationCategory *category = [UNNotificationCategory categoryWithIdentifier:@"categoryNoOperationAction" actions:array intentIdentifiers:@[] options:UNNotificationCategoryOptionCustomDismissAction];
